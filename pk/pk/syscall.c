...
uintptr_t sys_mmap(uintptr_t addr, size_t length, int prot, int flags, int fd, off_t offset)
{
  ...
  offset <<= 12;
#endif
  printk("addr %lx length %ld prot %d flags %d \n", addr, length, prot, flags);
...
static int sys_aes_start_decry(uint32_t *ct, uint32_t *st, uint32_t key_sel)
{
    ...
    register uint32_t  a7 asm ("a7") = (uint32_t)(SBI_AES_START_ENCRYPT);
...
static int sys_aes_start_encry(uint32_t *pt, uint32_t *st, uint32_t key_sel)
{
...
static int sys_aes_wait()
{


    register uint32_t  a7 asm ("a7") = (uint32_t)(SBI_AES_WAIT);
...
static int sys_aes_data_out(uint32_t *t, uint32_t key_sel)
{
    uint32_t volatile sct[4] = {0,0,0,0};

    register uintptr_t a0 asm ("a0") = (uintptr_t)(sct);
    register uint32_t  a7 asm ("a7") = (uint32_t)(SBI_AES_DATA_OUT);
...
static int sys_dma_copy(uint32_t sAddr, uint32_t dAddr, uint32_t length, uint32_t wait)
{

    uint32_t volatile ssAddr = sAddr ; 
    uint32_t volatile sdAddr = dAddr ; 
    uint32_t volatile slength = length ; 
    uint32_t volatile swait = wait ; 

    register uint32_t a0 asm ("a0") = (uint32_t)(ssAddr);
    register uint32_t a1 asm ("a1") = (uint32_t)(sdAddr);
...
static void sys_tlb_flush()
{
    asm volatile ("sfence.vma");
}

static int sys_dma_copy_to_perif(uint32_t *sAddr, uint32_t dAddr, uint32_t length, uint32_t wait)
{
...
static int sys_dma_copy_from_perif(uint32_t sAddr, uint32_t *dAddr, uint32_t length, uint32_t wait)
{
...
static int sys_hmac_hash(char *pString, uint32_t *hash, uint32_t *key)
{

    char volatile spString[500];
    uint32_t volatile shash[SHA256_HASH_WORDS]; 
    uint32_t volatile skey[8] = {key[0], key[1], key[2], key[3],key[4], key[5], key[6], key[7]} ; 
    ...
    register uintptr_t a0 asm ("a0") = (uintptr_t)(spString);
    register uintptr_t a1 asm ("a1") = (uintptr_t)(shash);
    register uintptr_t a2 asm ("a2") = (uintptr_t)(skey);
...
static int sys_cmp(uint32_t *expectedData, uint32_t receivedData, uint32_t block_size, uint32_t *equality)
{
    ...
    uint32_t volatile spt[32] ; 

    uint32_t volatile sst = receivedData; 
    ...
    register uintptr_t a0 asm ("a0") = (uintptr_t)(spt);
    register uint32_t a1 asm ("a1") = (uint32_t)(sst);
...
long do_syscall(long a0, long a1, long a2, long a3, long a4, long a5, unsigned long n)
{
  const static void* syscall_table[] = {
    ...
    [SYS_AES_START_ENCRY] = sys_aes_start_encry,    
    [SYS_AES_START_DECRY] = sys_aes_start_decry,    
    [SYS_AES_WAIT] = sys_aes_wait,    
    [SYS_AES_DATA_OUT] = sys_aes_data_out,    
    [SYS_SHA256_HASH] = sys_sha256_hash,    
    [SYS_HMAC_HASH] = sys_hmac_hash,    
    [SYS_DMA_COPY] = sys_dma_copy,       
    [SYS_FLUSH] = sys_tlb_flush,       
