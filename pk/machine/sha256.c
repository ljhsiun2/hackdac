...
int sha256_hashString(char *pString, uint32_t *hash)
{
  char *ptr = pString;
    ...
    while(!done) {
        ...
        // Send the message
        while (readFromAddress((uint32_t *)sha256, SHA256_READY) == 0) {
            do_delay(SHA256_READY_DELAY); 
        }

        writeMulticharToAddress((uint32_t *)sha256, SHA256_TEXT_BASE, message, SHA256_TEXT_BYTES); 

        // start the hashing
        if(firstTime) {
            //strobeInit();
            writeToAddress((uint32_t *)sha256, SHA256_NEXT_INIT, 0x1); 
            writeToAddress((uint32_t *)sha256, SHA256_NEXT_INIT, 0x0); 
            firstTime = 0;
        } else {
            //strobeNext();
            writeToAddress((uint32_t *)sha256, SHA256_NEXT_INIT, 0x2); 
            writeToAddress((uint32_t *)sha256, SHA256_NEXT_INIT, 0x0); 
        }
        ...
        while (readFromAddress((uint32_t *)sha256, SHA256_VALID) == 0) {
        ...
        if (addedBytes > SHA256_TEXT_BYTES) {
            ...
            writeMultiToAddress((uint32_t *)sha256, SHA256_TEXT_BASE, (uint32_t *)(message + SHA256_TEXT_BYTES), SHA256_TEXT_WORDS); 
    ...
    // Read the Hash
    readMultiFromAddress((uint32_t *)sha256, SHA256_HASH_BASE, hash, SHA256_HASH_WORDS); 
